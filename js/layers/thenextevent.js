addLayer("d", {
    ...
    ["display-text", function(){return "You have "+ format(player.p.points) +" prestige points [Point Gain: ("+ format(tmp.pointGen)+"/s)]"},{}],
    ["display-text", function(){if (player.u.population.lte(100) && inChallenge('u', 14)) return "<red>WARNING: Your Population is under 100!</red>"},{}],
    ...
    challenges: {
        ...
        17: {
            name: "Screwing Dust",
            challengeDescription(){
            return "Divide Point gain based on Prestige Points, Dust Requirement is gravely increased<br> (Nerf Effect: /" + format(player.p.points.pow(0.7).add(1)) + ")<br>Advice: Avoid prestiging until the 9th P-Challenge"},
        ...
        18: {
            name: "Dusty Sandbox",
            challengeDescription(){
            return "All P-Challenges are unlocked; each complete P-Challenge nerfs point gain<br> (Nerf Effect: /" + format(new Decimal.pow(3.72, tmp.d.challengeLook).max(1)) + ')<br> You have completed ' + formatWhole(tmp.d.challengeLook) + ' P-Challenges'},
            canComplete: function() {return player.p.points.gte(7500)},
            goalDescription: "7,500 Prestige Points",
