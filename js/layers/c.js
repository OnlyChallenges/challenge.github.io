addLayer("c", {
 ...
 upgrades: {
    ...
    11: {
        ...
        cost: new Decimal(1),
        effect() {
            return (player[this.layer].total.max(1).add(1.3).pow(0.13)).max(1).min(500);
        },
        effectDisplay() {
	...
	12: {
        ...
        cost: new Decimal(5),
        effect() {
            return (player[this.layer].total.max(1).add(1.45).pow(0.17)).max(1).min(25);
	...
	13: {
        ...
        cost: new Decimal(15),
        effect() {
            return (player[this.layer].total.max(1).add(1.25).pow(0.19)).max(1).min(150);
        },
        effectDisplay() {
        ...
        unlocked(){
            return hasUpgrade("c", 12) || hasUpgrade("c",11);
        },
    },
	14: {
	    ...
	    cost: new Decimal(50),
        unlocked(){
            return hasUpgrade("c", 13) || hasUpgrade("c",12);
        },
    },
    15: {
        ...
        unlocked(){
            return hasUpgrade("c", 14) || hasUpgrade("c",13);
        },
    },
    21: {
        title: "Solarfrost",
