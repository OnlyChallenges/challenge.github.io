addLayer("p", {
    ...
    gainMult() { // Calculate the multiplier for main currency from bonuses
        ...
        if (hasChallenge('d', 17)) mult = mult.times(player.d.points.pow(0.24).add(1))
        if (inChallenge('u', 13)) mult = mult.times(new Decimal.pow(1.77, tmp.u.challengeSafe).max(1))
        return mult
    },
    ...
    tabFormat: ["main-display", "prestige-button",["display-text", function(){return "You have "+ format(player.points) +" points ("+ format(tmp.pointGen)+"/s)"},{}],["display-text", function(){if (player.u.population.lte(100)) return "<red>WARNING: Your Population is under 100!</red>"},{}],"blank","challenges"],
    challenges: {
        11: {
            name: "The Beginning",
